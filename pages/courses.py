...
def register_callbacks(app):
    ...
    def open_modal(new_click, edit_clicks, _):
        triggered = ctx.triggered_id

        if triggered == "open-course-modal":
            return _course_modal(), None
        ...
        if isinstance(triggered, dict) and triggered.get("type") == "edit-course-btn":
            ...
            try:
                ...
                return _course_modal(c), course_id
            finally:
                db.close()
    ...
    @app.callback(
        ...
        Output("course-modal-container", "children", allow_duplicate=True),
    ...
    def save_course(n_clicks, code, label, hours, teacher, t_email, desc, color, edit_id, refresh):
        ...
        if not code or not label:
            return alert_msg("Code et libellé obligatoires.", "danger", "⚠️"), no_update, no_update
        ...
        try:
            if edit_id:
                ...
                if db.query(Course).filter_by(code=code).first():
                    return alert_msg(f"Le code {code} existe déjà.", "danger", "⚠️"), no_update, no_update
                c = Course()
                db.add(c)
    ...
    def delete_course(n_clicks_list, refresh):
        ...
        try:
            ...
            if c:
                ...
                db.commit()
            return alert_msg("Cours supprimé.", "warning", "🗑️"), (refresh or 0) + 1
        except Exception as e:
            db.rollback()
...
def _course_card(c: Course) -> html.Div:
    ...
    return html.Div(className="card", style={"borderTop":f"4px solid {c.color}"}, children=[
        html.Div(className="flex-between", style={"marginBottom":"12px"}, children=[
            ...
            html.Div(className="flex-center gap-8", children=[
                html.Button("✏️", id={"type":"edit-course-btn","index":c.id},
                           className="btn btn-sm btn-outline btn-icon", n_clicks=0),
                html.Button("🗑️", id={"type":"delete-course-btn","index":c.id},
                           className="btn btn-sm btn-outline btn-icon", n_clicks=0),
            ]),
        ]),
    ...
    return html.Div(className="modal-overlay", children=[
        html.Div(className="modal-box", children=[
            html.Div(className="flex-between", style={"marginBottom":"24px"}, children=[
                ...
                html.Button("✕", id="close-course-modal", className="btn btn-outline btn-icon", n_clicks=0),
            ]),
            ...
            html.Div(className="flex-center gap-12", style={"marginTop":"24px","justifyContent":"flex-end"}, children=[
                html.Button("Annuler", id="close-course-modal", className="btn btn-outline", n_clicks=0),
                html.Button(
